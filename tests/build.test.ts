...
describe("Builder Integration", () => {
	...
	it("should successfully build a tree and ignore non-Roblox files", () => {
		...
		(
			spyOn(fs, "readdirSync") as Mock<(dir: string) => any[]>
		...
		expect(resultTree.ServerScriptService.systems.Combat).toBeDefined();
		expect(resultTree.ServerScriptService.systems.Combat.$path).toBe(
	...
	it("should successfully merge files from multiple source directories into single containers", () => {
		...
		const baseTree: RojoTree = { name: "test-game", tree: {} };
		const config: RotreeConfig = { source: ["src/core", "src/chapter1"] };
		const env: Environment = { isTsProject: false, isDarkluaProject: false };
		const cliArgs: CliArgs = {};
	...
	it("should route files based on marker files instead of folder names", () => {
		...
		expect(resultTree.ServerScriptService.Database.query).toBeDefined();
		expect(resultTree.ServerScriptService.Database.query.$path).toBe(
